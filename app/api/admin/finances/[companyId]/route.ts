...
export async function GET(
  ...
  const { data: company, error } = await supabase
    .from("companies")
    .select("id,name,stripe_customer_id,stripe_subscription_id,subscription_status,worker_limit")
    .eq("id", companyId)
    .single();
  ...
  // Pull the same subscription + invoice view as BillingCard uses.
  ...
  const customer: StripeCustomerLike | null =
    stripeCustomerId
      ? ((await stripe.customers.retrieve(stripeCustomerId)) as unknown as StripeCustomerLike)
      : null;
  ...
  // Refunds: Stripe doesn't support listing refunds by customer directly.
  // V1: show refunded charges for this customer (if any), capped.
  let refundedCharges: StripeChargeLike[] = [];
  if (stripeCustomerId) {
    try {
      const charges = await stripe.charges.list({ customer: stripeCustomerId, limit: 50 });
      const data = (charges.data ?? []) as unknown as StripeChargeLike[];
      refundedCharges = data
        .filter((c) => (c.amount_refunded ?? 0) > 0 || c.refunded === true)
        .slice(0, 10);
    } catch {
      refundedCharges = [];
    }
  ...
  return NextResponse.json({
    ...
    stripe: {
      ...
      refundedCharges: refundedCharges.map((c) => ({
        id: c.id,
        created: c.created,
        amount: c.amount,
