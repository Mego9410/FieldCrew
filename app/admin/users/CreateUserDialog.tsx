...
export function CreateUserDialog() {
  ...
  const [userPays, setUserPays] = useState(false);
  ...
  const create = async () => {
    ...
    try {
      ...
      setSetPasswordLink(json.setPasswordLink ?? null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <Card className="w-full max-w-xl rounded-xl p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="mt-0.5 text-sm text-fc-muted">
                  Creates company + owner auth user. Returns a magic link and a set-password link.
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <div className="flex items-center justify-between gap-3 rounded-lg border border-fc-border bg-fc-surface-muted px-3 py-2">
                <div>
                  <div className="text-sm font-semibold text-fc-brand">
                    User pays?
                  </div>
                  <div className="text-xs text-fc-muted">
                    If enabled, the account will require Stripe checkout to activate.
                  </div>
                </div>
                <label className="inline-flex items-center gap-2 text-sm font-medium text-fc-brand">
                  <input
                    type="checkbox"
                    checked={userPays}
                    onChange={(e) => setUserPays(e.target.checked)}
            ...
            {magicLink || setPasswordLink ? (
              <div className="mt-4 space-y-3 rounded-lg border border-fc-border bg-fc-surface-muted p-3">
                {magicLink ? (
                  <div>
                ...
                {setPasswordLink ? (
                  <div>
                    ...
                    <div className="mt-2 flex gap-2">
                      <button
                        type="button"
                        onClick={() => navigator.clipboard.writeText(setPasswordLink)}
                        className="rounded-lg border border-fc-border px-3 py-2 text-sm font-medium text-fc-brand hover:bg-white"
                      >
                        Copy
                      </button>
                      <button
                        type="button"
                        onClick={() =>
                          window.open(setPasswordLink, "_blank", "noopener,noreferrer")
                        }
                        className="rounded-lg bg-fc-brand px-3 py-2 text-sm font-medium text-white hover:bg-fc-brand/90"
                      >
                        Open
